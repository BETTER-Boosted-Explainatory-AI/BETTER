...
def get_top_k_predictions(model, image, class_names, top_k=5):
    ...
    logger.debug(
        f"Top {top_k} predictions: {list(zip(top_labels, top_probs))}")
...
def _check_model_path(user_id: str, model_id: str, graph_type: str) -> Optional[str]:
    ...
    logger.info(
        f"Checking model path for user_id: {user_id}, model_id: {model_id}, graph_type: {graph_type}")
...
def _get_model_path(user_id: str, model_id: str) -> Optional[str]:
    ...
    try:
        ...
        if 'Contents' in response:
            ...
            logger.debug(
                f"S3 prefix {s3_prefix} does not exist in bucket {S3_BUCKET}")
            return None
...
def read_json_from_s3(bucket_name: str, s3_key: str) -> Any:
    ...
    try:
        ...
        logger.error(
            f"Error reading JSON from S3 ({bucket_name}/{s3_key}): {e}")
        raise
...
def construct_model(model_path: str, dataset_config: Dict[str, Any]) -> Model:
    ...
    logger.info(
        f"Loading model {model_path} for dataset {dataset_config['dataset']}")
    ...
    if not s3_file_exists(bucket_name, s3_key):
        raise FileNotFoundError(
            f'Model s3://{bucket_name}/{s3_key} does not exist')
...
def query_model(top_label, model_id, graph_type, user):
    ...
    if model_info is None:
        ...
        model_path = get_model_specific_file(
            user.get_user_folder(), model_info, graph_type, "graph_folder")
    ...
    consistency = dendrogram.find_name_hierarchy(
        dendrogram.Z_tree_format, top_label)
...
def query_predictions(model_id, graph_type, image, user):
    ...
    model_s3_key = get_model_specific_file(
        user.get_user_folder(), model_info, graph_type, "model_file")

    if s3_file_exists(S3_BUCKET, model_s3_key):
        # Load model from S3
        current_model = load_model_from_s3(S3_BUCKET, model_s3_key)
        logger.debug(
            f"Model loaded successfully from 's3://{S3_BUCKET}/{model_s3_key}'.")
    else:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Model file does not exist in S3"
        )

    preprocessed_image = preprocess_loaded_image(current_model, image)
    predictions = get_top_k_predictions(
        current_model, preprocessed_image, labels)
...
def get_user_models_info(user, model_id):
    ...
    if s3_file_exists(S3_BUCKET, s3_models_json_key):
        ...
        logger.debug(
            f"Models data loaded from s3://{S3_BUCKET}/{s3_models_json_key}")
    else:
        models_data = []
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def get_model_files(user_folder: str, model_info: dict, graph_type: str):
    ...
    logger.info(
        f"Getting model files for user folder: {user_folder}, model info: {model_info}, graph type: {graph_type}")
    ...
    if 'Contents' not in response:
        ...
        raise ValueError(
            f"Model graph folder s3://{S3_BUCKET}/{model_graph_folder} does not exist")
    ...
    if not dendrogram_file_exists:
        ...
        logger.debug(
            f"Dendrogram file s3://{S3_BUCKET}/{dendrogram_file} does not exist")
    ...
    if not detector_file_exists:
        ...
        logger.debug(
            f"Detector model file s3://{S3_BUCKET}/{detector_filename} does not exist")
    ...
    if not dataframe_file_exists:
        ...
        logger.debug(
            f"Dataframe file s3://{S3_BUCKET}/{dataframe_filename} does not exist")
    ...
    match file_type:
        ...
        case "Z_file":
            ...
            if not s3_file_exists(S3_BUCKET, file_path):
                logger.debug(
                    f"Z file s3://{S3_BUCKET}/{file_path} does not exist")
                file_path = None
        case "dendrogram":
            file_path = f"{model_graph_folder}/dendrogram.json"
            if not s3_file_exists(S3_BUCKET, file_path):
                logger.debug(
                    f"Dendrogram file s3://{S3_BUCKET}/{file_path} does not exist")
                file_path = None
        case "dataframe":
            file_path = f"{model_graph_folder}/edges_df.csv"
            if not s3_file_exists(S3_BUCKET, file_path):
                logger.debug(
                    f"Dataframe file s3://{S3_BUCKET}/{file_path} does not exist")
                file_path = None
        case _:
